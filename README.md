...
**To build it yourself** — needs a stable Rust toolchain, 1.85 or newer:

```
cargo install --path crates/aitch
```

**Windows only.** The window, the text rasteriser and the clipboard are
written directly against Win32 and GDI, so there is no Linux or macOS build —
not packaging that has yet to be done, but a backend that does not exist. A
second one could be written against the same interfaces
([`aitch-ui/src/platform/`](crates/aitch-ui/src/platform)); nothing above that
...
Syntax highlighting is hand-written per language and runs on its own thread, so a
...
The binary has no crate dependencies at all (`PLAN-ZERO-DEP.md`'s zero-
dependency rewrite) — `cargo tree --workspace` shows only Aitch's own four
crates. [`docs/third-party.md`](docs/third-party.md) covers the one thing
still worth a licence note: the bundled DejaVu Sans Mono font.
