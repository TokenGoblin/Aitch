...
# A checked-in asset. The generator that drew it from the dark theme was
# removed with the winit backend, so to change the icon, replace the file.
# Checked here because WiX reports a missing icon several minutes in.
$icon = Join-Path $PSScriptRoot 'aitch.ico'
if (-not (Test-Path $icon)) {
    throw "No icon at $icon. It is checked in -- restore it from git."
}
