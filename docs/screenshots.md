...
`packaging/windows/aitch.ico` is embedded in `aitch.exe` by
`crates/aitch/build.rs`, which is where Explorer, the Start menu and Alt-Tab
read it from.

The generator that drew it (`aitch-ui`'s `make_icon` example) and the raw
`assets/icon.rgba` it also wrote are both gone, removed alongside the winit
backend that was the only thing loading the raw form. The `.ico` is now a
checked-in asset like any other: to change it, replace the file. The window
class sets no icon of its own (`h_icon: 0`), so what the taskbar shows comes
from the executable's resource.
