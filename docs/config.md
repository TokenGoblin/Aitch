...
Only the Windows row applies to a build you can download: the editor is
Windows-only. The others are what `aitch-core` computes when it is built
elsewhere — it has no GUI dependencies and stays portable on purpose — and
are listed so that behaviour is written down rather than guessed at.
