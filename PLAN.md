...
**Target platforms:** Windows 10+. *(Originally Windows and Linux; the
zero-dependency rewrite replaced winit/wgpu with a hand-written Win32/GDI
backend and dropped Linux deliberately — see `PLAN-ZERO-DEP.md` §0.)*
**Language:** Rust
...
- ~~Linux: an AppImage, a `.deb`, a tarball and a Flatpak manifest.~~
  **Withdrawn.** All of it shipped in 0.1.3 and all of it was removed with
  the zero-dependency rewrite, which replaced winit/wgpu/cosmic-text with a
  Win32/GDI backend and left nothing to package. `packaging/linux/` and the
  release workflow's `linux-packages` job are gone; `git log` before the
  rewrite has them if they are ever wanted back.
