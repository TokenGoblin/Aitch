...
///
/// Kept in linear space rather than raw sRGB bytes because contrast-ratio
/// math (below) needs linear luminance, and computing that from bytes on
/// every comparison would be worse than converting once at construction.
...
impl Color {
    ...
    /// The inverse of [`Color::srgb`]: the 8-bit sRGB bytes this color came
    /// from (or the nearest ones, if it didn't).
    ///
    /// GDI, unlike a wgpu sRGB-format surface, does no gamma correction of
    /// its own — it wants the raw device bytes a theme color was authored in,
    /// not the linear light this type stores them as. This is the Phase 0
    /// bridge between the two; Phase 1's real color pipeline can target
    /// 8-bit sRGB directly and retire it.
...
/// The sRGB transfer function: linear light back to an 8-bit device byte.
...
mod tests {
    ...
    #[test]
    fn to_srgb_bytes_round_trips_through_linear_space() {
        for (r, g, b) in [
            (0, 0, 0),
            (255, 255, 255),
            (0x14, 0x16, 0x1a),
            ...
            assert_eq!(Color::srgb(r, g, b).to_srgb_bytes(), (r, g, b));
        }
    }
