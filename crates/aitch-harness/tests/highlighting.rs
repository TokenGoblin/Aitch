...
fn opening_another_language_starts_the_right_parser() {
    // JSON, not Python: PLAN-ZERO-DEP.md Phase 5 Wave 1 covers Rust, JSON,
    // TOML, Markdown and Bash first, and Python is Wave 2. Any two Wave 1
    // languages prove the same thing — the worker switches lexers — and
    // this one still needs no real Python lexer to exist.
    let scratch = Scratch::new();
    let rust = scratch.file("main.rs", "fn main() {}\n");
    // Written so quick open can find it; opened by name below, not by path.
    scratch.file("data.json", "{\"key\": true}\n");
    ...
    assert!(settle(&mut h));
    assert_eq!(token_at(&h, 0), Some(Token::Punctuation), "{{");
}
...
fn switching_files_colours_the_new_one_and_not_the_old_one() {
    ...
    // keeps its line states between requests. Nothing then told it the text
    // was a different document rather than a later version of the same one,
    // so it reused the old file's states and coloured the new file at the
    // old file's offsets.
    let scratch = Scratch::new();
