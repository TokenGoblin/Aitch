...
#[test]
fn the_phase_four_and_six_commands_admit_they_are_not_built() {
    // Better a key that says what it is than one that silently does nothing.
    for (chord, expect) in [
        ("^T", "quick open"),
        ("M-T", "toggle tree"),
        ("M-B", "buffer list"),
        ...
        let status = h.status();
        assert!(
            status.contains(expect) && status.contains("not built yet"),
            "{chord} said {status:?}"
        );
        assert_eq!(h.text(), "text", "{chord} changed the buffer");
    }
}
