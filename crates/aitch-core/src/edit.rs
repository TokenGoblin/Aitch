...
//!
//! This is the only file that calls a mutating [`PieceTable`] method. If you
//! are reaching for `PieceTable::insert` anywhere else, stop (see
//! `CLAUDE.md`).
...
/// Apply an edit to a piece table.
///
...
/// character. Comparing chars rather than bytes keeps it correct over
/// multi-byte text.
pub(crate) fn apply(rope: &mut PieceTable, edit: &Edit) {
    assert!(
    ...
    assert!(
        rope.slice_to_string(edit.at..edit.removed_end())
            .chars()
