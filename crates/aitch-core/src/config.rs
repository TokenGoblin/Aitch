...
impl ThemeChoice {
    ...
    fn parse(text: &str) -> Option<ThemeChoice> {
        match text {
...
impl FontConfig {
    ...
    fn from_value(value: &Value) -> Result<FontConfig, String> {
        let mut font = FontConfig::default();
        let entries = value.expect_table("font")?;

        for (key, value) in entries {
            match key.as_str() {
                "family" => font.family = Some(value.expect_str("font.family")?.to_string()),
                "size" => font.size = value.expect_float("font.size")? as f32,
                other => return Err(format!("unknown setting `font.{other}`")),
            }
        }
        Ok(font)
    }
...
impl Config {
    ...
    /// Read a parsed TOML document into a `Config`, defaults standing in for
    /// anything not mentioned — and an error, not a silent skip, for a key
    /// this schema does not recognize at all (the `#[serde(deny_unknown_
    /// fields)]` this replaced) or one whose value is the wrong shape.
    fn from_value(root: &Value) -> Result<Config, String> {
        ...
        for (key, value) in entries {
            match key.as_str() {
                "theme" => {
                    let text = value.expect_str("theme")?;
                    config.theme = ThemeChoice::parse(text)
                        .ok_or_else(|| format!("unknown theme `{text}`"))?;
                }
                "keymap" => config.keymap = value.expect_str("keymap")?.to_string(),
                "tab_width" => config.tab_width = value.expect_integer("tab_width")? as usize,
                "expand_tabs" => config.expand_tabs = value.expect_bool("expand_tabs")?,
                "line_numbers" => config.line_numbers = value.expect_bool("line_numbers")?,
                "whitespace" => config.whitespace = value.expect_bool("whitespace")?,
                "ignore" => config.ignore = value.expect_string_array("ignore")?,
                "font" => config.font = FontConfig::from_value(value)?,
