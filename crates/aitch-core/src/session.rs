...
///
/// A path is assumed never to contain a literal `\n` or `\t` -- a real
/// filesystem path never does in practice, and that assumption is what lets
/// [`Session`] use them as line- and field-separators. (Same spirit as
/// `matcher.rs`'s ASCII-only case folding note: a documented simplification,
/// not an oversight.)
...
impl Session {
    ...
    pub fn default_path() -> Option<PathBuf> {
        state_dir().map(|dir| dir.join("session.state"))
    }
    ...
    pub fn load_from(path: &Path) -> Option<Session> {
        ...
        let session = Session::decode(&text);
        if session.is_empty() {
            return None;
        }
        Some(session)
    ...
    pub fn save_to(&self, path: &Path) {
        ...
        let _ = std::fs::write(path, self.encode());
    }

    /// Serialize to a plain `key=value`-per-line format: one `root=`,
    /// `root_opened=`, and `active=` line, then one `file=` line per entry in
    /// `files`, each a tab-separated `path\tline\tcolumn`.
    ...
    /// Parse the `key=value` format, line by line. Tolerant by design, the
    /// same way `#[serde(default)]` used to be: a blank line, an unrecognized
    /// key, or a `file=` line that doesn't split into exactly path/line/column
    /// (or whose line/column don't parse as `usize`) is skipped rather than
    /// failing the whole parse, so a future field -- or garbage input -- just
    /// leaves the rest of the session at its default.
    fn decode(text: &str) -> Session {
        let mut session = Session::default();
        for line in text.split('\n') {
            if let Some(value) = line.strip_prefix("root=") {
                if !value.is_empty() {
                    session.root = Some(PathBuf::from(value));
                }
            } else if let Some(value) = line.strip_prefix("root_opened=") {
                session.root_opened = value == "1";
            } else if let Some(value) = line.strip_prefix("active=") {
                if let Ok(active) = value.parse() {
                    session.active = active;
                }
            } else if let Some(value) = line.strip_prefix("file=") {
                let parts: Vec<&str> = value.split('\t').collect();
                if let [path, line, column] = parts[..] {
...
impl Recovery {
    ...
    fn file_name(path: Option<&Path>, key: u64) -> String {
        match path {
            ...
            None => format!("scratch-{key:016x}.rec"),
        }
    }
    ...
    pub fn write(path: Option<&Path>, key: u64, text: &str) -> Option<PathBuf> {
        ...
        std::fs::write(&file, recovery.encode()).ok()?;
        Some(file)
    }

    /// Serialize to the on-disk format: a fixed two-line header -- the path
    /// (or an empty line if there is none), then `saved_at` -- followed by
    /// `text` completely unprocessed. `text` is someone's only copy of
    /// unsaved work, so nothing after the header may be escaped or
    /// reinterpreted; a round trip has to be byte-for-byte.
    ///
    /// `display()` uses OS-specific separators, matching the convention
    /// `describe()` already uses to turn a path into text.
    fn encode(&self) -> String {
        let path = self
            .path
            .as_ref()
            ...
            .unwrap_or_default();
        format!("{path}\n{}\n{}", self.saved_at, self.text)
    }

    /// Parse the on-disk format. `splitn(3, '\n')`, not `lines()`, which
    /// would also split the body apart: this keeps the third part -- `text`
    /// -- completely intact, including any `\n` or `\r\n` it contains.
    ///
    /// `None` when there are fewer than three parts -- a file missing its
    /// header, or empty -- which the caller treats the same way
    /// `pending_in`'s doc comment already describes: left where it is, not
    /// deleted, not panicked on.
    fn decode(text: &str) -> Option<Recovery> {
        let mut parts = text.splitn(3, '\n');
        let path = parts.next()?;
        let saved_at = parts.next()?;
        let body = parts.next()?;
    ...
    pub fn write_in(directory: &Path, path: Option<&Path>, key: u64, text: &str) -> PathBuf {
        ...
        std::fs::write(&file, recovery.encode()).expect("write");
        file
    }
    ...
    pub fn pending_in(directory: &Path) -> Vec<Recovery> {
        ...
        for entry in entries.filter_map(Result::ok) {
            ...
            if let Some(mut recovery) = Recovery::decode(&text) {
...
mod tests {
    ...
    #[test]
    fn a_recovery_file_round_trips() {
        let recovery = Recovery::detached(
        ...
        let decoded = Recovery::decode(&encoded).unwrap();
