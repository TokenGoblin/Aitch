...
impl Editor {
    ...
    pub fn offer_recovery_from(&mut self, candidates: Vec<Recovery>) -> bool {
        ...
        let current = self.workspace.active().buffer.snapshot();
    ...
    pub fn bracket_pair(&self) -> Option<(usize, usize)> {
        ...
        let at = std::iter::once(cursor)
            ...
            .find(|index| {
                *index < buffer.len_chars() && bracket_of(buffer.char_at(*index)).is_some()
            })?;

        let (partner, forward) = bracket_of(buffer.char_at(at))?;
        let opener = buffer.char_at(at);

        let mut depth = 0usize;
        let mut index = at;
        loop {
            index = if forward {
                index.checked_add(1).filter(|i| *i < buffer.len_chars())?
            } else {
                index.checked_sub(1)?
            };
            ...
            if self.is_in_text_or_comment(buffer, index) {
                continue;
            }

            let c = buffer.char_at(index);
            if c == opener {
                depth += 1;
            } else if c == partner {
                if depth == 0 {
    ...
    fn is_in_text_or_comment(&self, buffer: &Buffer, char_index: usize) -> bool {
        let Some(highlights) = self.highlights() else {
            return false;
        };
        let byte = buffer.char_to_byte(char_index);
        matches!(
    ...
    /// Ask for colour for a document the lexer has not seen before.
    ///
    /// The worker survives a switch between two files of the same language,
    /// and it keeps its line states between requests — so without this, it
    /// was handed a different document with no edits to explain the
    /// difference, and reused the previous file's states at offsets that
    /// meant nothing in the new one.
    ...
    fn request_highlights_inner(&mut self, whole_buffer: bool, fresh: bool) {
        ...
        if let Some(syntax) = self.syntax.as_mut() {
            syntax.request(&self.workspace.active().buffer, edits, range, fresh);
        }
    }
    ...
    fn search_from_prompt(&mut self) {
        ...
        if let Some(found) = search::find(
            &self.workspace.active().buffer.snapshot_chars(),
            &query,
            origin,
        ) {
    ...
    fn continue_replace(
        ...
        let done = match answer {
            ...
            Answer::All => {
                ...
                while let Some(found) = search::find(
                    &self.workspace.active().buffer.snapshot_chars(),
                    &query,
                    from,
                ) {
