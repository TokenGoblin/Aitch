...
//!
//! This used to be ripgrep's own machinery — `ignore` to walk, `grep-regex`
//! to match, `grep-searcher` to read. As of `PLAN-ZERO-DEP.md` Phase 6 it is
//! hand-written instead: [`crate::walk::walk_streaming`] to walk (the same
//! [`crate::project::IgnoreRules`] `Tree`/`PathIndex` already use),
//! [`crate::matcher::LiteralMatcher`]/[`crate::regex::Regex`] to match, and a
//! plain byte-scan to read — same shape of work, same guarantees, no
...
use crate::matcher::{LiteralMatcher, Matcher};
use crate::project::IgnoreRules;
...
impl ProjectSearch {
    ...
    {
        let case_sensitive = pattern.is_case_sensitive();
        let matcher: Arc<dyn Matcher> = if pattern.regex {
            Arc::new(
                Regex::new(&pattern.text, case_sensitive)
                    .map_err(|e| SearchError::BadPattern(e.to_string()))?,
            )
        } else {
            Arc::new(LiteralMatcher::new(&pattern.text, case_sensitive))
        };
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                walk::walk_streaming(&root, &rules, &thread_cancelled, |path| {
...
/// Search one file, sending hits on in batches as they're found.
///
/// Replaces `grep-searcher`'s `Searcher`/`Sink`: this decodes the whole file
/// at once through [`crate::fileio::load`] — the same encoding detection
/// `Document::open`/`save` use, so a UTF-16 or Latin-1 file is searched as
/// the text it actually is rather than as raw bytes `grep-searcher` would
/// only get right via its own separate encoding transcoding — and then
/// scans the decoded text line by line by hand.
///
/// An unreadable or undecodable file is skipped, not fatal — one permission
/// problem or malformed file should not end the search — and so is a binary
/// one: a NUL character anywhere in the *decoded* text means the whole file
/// is skipped, not just truncated at that byte. (Checking the decoded text
/// rather than the raw bytes is what tells a real UTF-16 file, whose raw
/// bytes are full of NULs by design, from an actually-binary one, whose
/// decoded text still has them because there was never real text there to
/// decode.)
...
fn search_file(
    ...
    relative: PathBuf,
    matcher: &dyn Matcher,
    sender: &mpsc::Sender<Vec<Hit>>,
    cancelled: &AtomicBool,
    found: &AtomicUsize,
    on_hits: &(dyn Fn() + Send + Sync),
) {
    let Ok(loaded) = crate::fileio::load(path) else {
        return;
    };
    if loaded.text.contains('\0') {
        return;
    }
    ...
    for line in loaded.text.split_inclusive('\n') {
        if line.is_empty() {
            continue;
        }
        line_number += 1;
        ...
        if found.fetch_add(1, Ordering::Relaxed) >= MAX_HITS {
            // "Found enough" is just another reason to stop, so it reuses
            // the same flag every other worker's walk already checks —
            // no separate signal needed to make the whole search wind down.
        ...
        if batch.len() >= BATCH_SIZE || last_sent.elapsed() >= BATCH_INTERVAL {
            ...
            if sender.send(sent).is_ok() {
                on_hits();
            }
        }
    }

    if !batch.is_empty() && sender.send(batch).is_ok() {
        on_hits();
    }
}
