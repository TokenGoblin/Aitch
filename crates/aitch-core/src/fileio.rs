...
//!
//! # Genuinely dependency-free — but it wasn't quite, until this was checked
//!
//! Encoding detection and en/decoding above (`detect`, `sniff_utf16`,
//! `decode`, `encode`) touch nothing but `std` — bytes and `str`/`String` all
//! the way through. [`load_bytes`] also works out the file's *dominant line
//! ending* via
//! [`LineEnding::dominant`](crate::line_ending::LineEnding::dominant), which
//! used to take a `&ropey::Rope` for no reason beyond calling `.chars()` on
//! it. The Phase 2 Track B audit found this incidental dependency — this file
//! was not the zero-dependency island PLAN-ZERO-DEP.md's Phase 2 line
//! assumed — and `dominant` now takes a plain `&str` instead.
...
pub fn detect(bytes: &[u8]) -> Encoding {
    ...
    // Unlike the UTF-16 sniff above, this checks every byte, not just the
    // first `SNIFF_LIMIT`: validating UTF-8 is one linear pass with no
    // realistic cost to bound, and this file's whole reason for existing is
    // that a wrong guess here is worse than a slow one.
...
mod tests {
    ...
    #[test]
    fn a_balanced_nul_pattern_does_not_trigger_the_utf16_sniff() {
        // Plenty of NULs (half the bytes), but split evenly across even and
        // odd offsets rather than overwhelmingly one side — the "weak or
        // balanced" case `sniff_utf16`'s own doc comment says must return
        // `None` rather than pick a side. This is still valid UTF-8 (0x00
        // and b'A' both are), so it must fall through to that, not to a
        // UTF-16 guess.
        let bytes: Vec<u8> = (0..20u8)
            .map(|i| if i % 4 == 0 || i % 4 == 1 { 0 } else { b'A' })
            .collect();
        assert_eq!(detect(&bytes), Encoding::UTF8);
    }

    #[test]
    fn a_sniffed_utf16_file_with_no_bom_can_still_be_truncated() {
        // Same trap as `a_truncated_utf16_file_is_an_error`, but reached via
        // the no-BOM sniffing path instead of a BOM, to confirm `decode`
        // catches the odd byte count regardless of how `detect` got there.
        let mut bytes: Vec<u8> = "hello, world\nsecond line\n"
            .encode_utf16()
            .flat_map(|u| u.to_le_bytes())
            .collect();
        bytes.push(0x41); // one stray trailing byte, no partner
        assert_eq!(
            detect(&bytes),
            Encoding::new(Charset::Utf16Le, false),
            "sniffing should still see the pattern in the well-formed prefix"
        );
    ...
    #[test]
    fn an_unpaired_utf16_surrogate_is_an_error_not_a_silent_substitution() {
        // 0xD800 alone (a high surrogate with no low surrogate to follow it)
        // is not valid UTF-16. Exercised directly against `decode` with a
        // known `Encoding`, rather than through `detect`'s heuristics, since
        // this is about `String::from_utf16`'s own rejection, not sniffing.
        let bytes = 0xD800u16.to_le_bytes();
        let error = decode(&bytes, Encoding::new(Charset::Utf16Le, false)).unwrap_err();
        assert!(error.to_string().contains("unpaired UTF-16 surrogate"));
    }

    #[test]
    fn latin1_encode_boundary_is_exactly_0xff() {
        // The fallback's whole justification is "every byte is a character",
        // so the boundary between "fits" and "does not" must sit exactly at
        // 0xFF/0x100, not somewhere padded for safety.
        let encoding = Encoding::new(Charset::Latin1, false);
        assert_eq!(encode("\u{FF}", encoding).unwrap(), vec![0xFF]);
        let error = encode("\u{100}", encoding).unwrap_err();
        assert!(error.to_string().contains("cannot be written as Latin-1"));
    }
